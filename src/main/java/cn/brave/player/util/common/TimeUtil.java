...
public class TimeUtil {
    ...
    public static long toMillis(String time) {
        ...
        for (index = end - 1; index >= 0; index--) {
            ...
            if (time.charAt(index) == ':') {
                ...
                millis += (long) Integer.parseInt(time.substring(index + 1, end)) * convert;
                // 转换倍数 = 原转换倍数 * 60 (叠加过程)
                convert *= 60;
                // 截取字符串结束位置=当前位置
                end = index;
        ...
        // 毫秒值 = 原毫秒值 + 新的时间值 * 转换倍数 (叠加过程)
        return millis + (long) Integer.parseInt(time.substring(0, end)) * convert;
    }
    ...
    public static long toTimeMillis(String time) {
        ...
        for (int index = times.length - 2; index >= 0; index--) {
            millis += ((long) Integer.parseInt(times[index]) * increment);
            increment *= 60;
        }
        ...
        if (times != null && times.length == 2) {
            ...
            millis *= 1000;
            millis += Integer.parseInt(times[1]) * 10L;
        } else {
    ...
    public static long toTimeMillis(Pattern pattern, String time) {
        String[] times = pattern.split(time);
        long millis = Integer.parseInt(times[0]) * 60L;
        millis += Integer.parseInt(times[1]);
        millis *= 1000;
        if (times.length >= 3) {
            millis += Integer.parseInt(times[2]) * 10L;
        }
        return millis;
    }
